...
const BrainNote = ({ note }) => {
  ...
  if (note.inboundReferenceNotes != null) {
    ...
    references = note.inboundReferenceNotes.map((reference) => (
      <RefLink
        ...
        key={reference.slug}
      >
        <div className={"p-2 ml-2 mb-2 text-gray-600 rounded-md hover:translate-x-1 hover:bg-opacity-50 transform hover:text-gray-700 hover:bg-gray-400 transition duration-500 dark:transform hover:text-gray-200 hover:bg-gray-800"}>
    ...
    if (references.length > 0) {
      referenceBlock = (
        <>
          ...
          <div className={"mb-4 border-l-2 border-gray-400 hover:border-gray-500 dark:border-gray-800 hover:border-gray-600"}>{references}</div>
          <hr className="mx-auto pt-2 mt-6 border-gray-400 dark:border-gray-800"/>
        </>
      );
    }
  }
    ...
    note.outboundReferenceNotes
      ...
      .forEach((ln, i) => {
        popups[ln.slug] = (
          <div
            id={ln.slug}
            className={`${popupStyles} bg-gray-200 text-black dark: bg-gray-800 text-white`}
          >
  ...
  return (
    <MDXProvider components={{ ...components, a: AnchorTagWithPopups }}>
      ...
      <div className="refs-box bg-opacity-50 text-gray-600 rounded-lg p-4 pt-2 bg-gray-200 dark:bg-gray-800">
        {referenceBlock}
