    ...
    if (references.length > 0) {
      referenceBlock = (
        <>
          ...
          <div className={
            `mb-4 border-l-2
            ${ ColourMode() ? 'border-gray-800 hover:border-gray-600' : 'border-gray-400 hover:border-gray-500'} transition duration-500
            `
          }>{references}</div>
          <hr className=
          {`mx-auto pt-2 mt-6 ${ ColourMode() ? 'border-gray-800' : 'border-gray-400'}`}/>
        </>
      );
    }
  }
    ...
    note.outboundReferenceNotes
      ...
      .forEach((ln, i) => {
        popups[ln.slug] = (
          <div
            id={ln.slug}
            className={`${popupStyles} ${ ColourMode() ? 'bg-gray-800 text-white' : 'bg-gray-200 text-black'}`}
          >
  ...
  return (
    <MDXProvider components={{ ...components, a: AnchorTagWithPopups }}>
      ...
      <div className={
        ...
        ${ ColourMode() ? 'bg-gray-800' : 'bg-gray-200'}
        `
      }>
        {referenceBlock}
